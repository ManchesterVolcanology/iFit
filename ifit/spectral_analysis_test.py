...
class Analyser():
    ...
    def pre_process(self, spectrum):
        ...
        # Define the pixel wavelength limits
        lims = np.zeros([len(x), 2])

        for n in range(len(x)):

            if n == 0:
                lims[n] = [x[n] - (x[n+1] - x[n])/2,
                           x[n] + (x[n+1] - x[n])/2]
                
            elif n == len(x)-1:
                lims[n] = [x[n] - (x[n] - x[n-1])/2,
                           x[n] + (x[n] - x[n-1])/2]
        ...
        self.common['pixel_bins'] = lims[self.common['fit_idx']]
...
#==============================================================================
    ...
    def fwd_model(self, x, *p0):
        ...
        *x0, list
        ...
        fit = np.zeros(len(x))
        for n, bins in enumerate(self.common['pixel_bins']):
            idx = np.where(np.logical_and(shift_model_grid >= bins[0],
                                          shift_model_grid < bins[1]))
            
            fit[n] = np.average(F_conv[idx])
    
        # Interpolate onto measurement wavelength grid
        # fit = griddata(shift_model_grid, F_conv, x, method='linear')
